...
export default {
  ...
  _updateChangeLog(attributes) {
    ...
    if (attributes.dryRun) {
      let preview = changeLog.preview(attributes.newVersion, attributes.description, attributes.commitsToInclude)
      preview = preview.replace('\n', '<br>')
      this.publishProgressView.messageUser(`[DRY RUN]: would have changed ${changeLogFile} to:\n${preview}`)
    } else {
      changeLog.update(attributes.newVersion, attributes.description, attributes.commitsToInclude)
    }
  },
