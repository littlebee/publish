...
module.exports = (Publish = {
  ...
  activate(state) {
    ...
    return this.publishProgressView.on("cancel", this._onCancel);
  },
  ...
  version(versionMethod=null){
    ...
    const [commits, lastVersionTag] = Array.from(this.getCommitsSinceLastTag(packageFile));
  ...
  _bumpVersionPart(partIndex, currentVersion) {
    ...
    if (partIndex < parts.length) {
      for (let start = partIndex + 1, i = start, end = parts.length, asc = start <= end; asc ? i < end : i > end; asc ? i++ : i--) { 
        parts[i] = "0";
      }
    }
  ...
  _isAtomPackage() {
    return (__guard__(this.npmPackage != null ? this.npmPackage.engines : undefined, x => x.atom) != null);
  },
  ...
  _onPublishFail(code) {
    ...
    return false;
  }   // don't continue executing commands
});
