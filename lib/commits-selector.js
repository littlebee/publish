...
export default class CommitsSelector extends React.Component {
  ...
  render() {
    return (
      <div>
        <h5 className="publish--commit-count">{this.props.commits.length || 'No'} Commits Since Last Tag</h5>
        <div><label>Include in change log?</label></div>
        <div className="publish--commits">
          {this._renderCommits()}
        </div>
        <div className="publish--scroll-shadow select-controls">
  ...
  _renderCommit(commit, index) {
    ...
    return (
      <div className="publish--commit" key={index}>
        <label><input key={index} type="checkbox" checked={isSelected} onChange={_onChange}/>
          {commit.hash}</label> by {commit.authorName} {commit.relativeDate}: {commit.message}
