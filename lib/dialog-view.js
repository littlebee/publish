...
export default class DialogView extends React.PureComponent {
  ...
  constructor(props) {
    super(props)
    autobind(this, '_onSaveClick', '_onCancelClick', '_onBackdropClick', '_onDocKeyup')
  }

  componentDidMount() {
    this._initFocus()
    // I tried to use atom.commands.add('core:cancel', ...) but it didn't work
    // Also adding the event listener directly to document and that didn't work
    // Thanks @mark_hahn for the tip to listen on body
    // https://discuss.atom.io/t/how-to-listen-and-prevent-keydown-event/22867
    document.querySelector('body').addEventListener('keyup', this._onDocKeyup)

  }

  componentWillUnmount() {
    document.querySelector('body').removeEventListener('keyup', this._onDocKeyup)
  }
      ...
      <div className="publish--modal-backdrop" onClick={this._onBackdropClick}>
        <div className="publish--modal">
          <div className="publish--modal-header">
            <h5 className="publish--dialog-title">{this.props.title}</h5>
          </div>
          <div className="publish--modal-body native-key-bindings">
